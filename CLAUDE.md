...
**City hub pages** (`digital-marketing-agency-in-<slug>/`): dedicated long-form "The #1 Digital Marketing Agency in [City]" landing pages for every service area. 10 of them: toronto (HQ), vancouver, calgary, montreal, london (London **UK**, the capital, British spelling; the existing per-city London service pages are also UK), new-york, los-angeles, chicago, austin, tampa. (Houston + San Antonio were removed and Austin added on Jake's call; to change the set, edit `ORDER`/`META` in `/tmp/citygen2.py`, the `slugs` list in the linkmap builder, and the footer-band tile sweep.) Each is ~2,300-2,930 words, built to rank (modeled on top-ranking agency city pages). **Hero is a warm city greeting, NOT a "#1" boast** (Jake's call, inspired by The Influence Agency's location pages): the big display line reads "[City], let's build something that scales." (city accented) but it is a styled <p>, NOT the H1. The actual H1 is the small keyword eyebrow line "[City] Digital Marketing Agency" (Mo's call, July 2026: keyword phrase must be the H1 on every location page). Keywords also live in the title tag ("Digital Marketing Agency in [City] | Consultus Digital") and the meta description, never claiming to be #1 or physically located in the city. Sections: hero, dark thesis band, 3-paragraph localized "Why [City]" market analysis, six in-depth service blocks (each with copy + 3 bullets), industries, 4-step process, dark proof scoreboard (real case-study numbers: La Vie 512, BookSeats 8.10×, Dragonscale 13.2 MER, Zayouna $72.70), differentiators, **areas-served** chips (real neighborhoods for local SEO), 9-question localized FAQ, CTA. Also emits **FAQPage + ProfessionalService JSON-LD** (with `areaServed`). **Internal-link mesh:** each service block links to that city's actual service page (per-city where it exists, e.g. `/locations/toronto/google-ads-agency-in-toronto/`, `/cro-agency-in-<city>/`, `/facebook-advertising-agency-in-<city>/`; general `/google-ads/` etc. as fallback), proof → case studies, and an "other service areas" grid → all 10 sibling city pages + `/locations/`. The `/locations/` hub has a prominent grid linking all 11. New, on-brand `.dm-*`/`.dms-*` CSS injected per page (`#dmcity` style block). Two-stage build: a Workflow (research top-ranking pages → 11 per-city writer agents) wrote unique localized JSON to `/tmp/citydata/<slug>.json` (no fabricated metrics, real geography), then `/tmp/citygen2.py` assembled pages from the `cro-agency-in-los-angeles` shell using `/tmp/citydata/_linkmap.json` (per-city service-page link resolution). Rerun citygen2.py to regenerate. The **footer Service Areas band** city tiles link here. Current tile style (Jake's "11 SMES EMBEDDED" reference): light bone cards (`#F1EDE4`) with a **sand-grain texture** overlay (`::before` feTurbulence noise data-URI, mix-blend multiply, opacity .45), blue skyline glyph, bold dark city name; Toronto = lead tile with ink skyline + yellow "HQ" badge via `.svc-area.is-hq`. (This replaced earlier dark/flat treatments. The CSS block is bounded by `/* Service-areas band: ... */ ... /* end service-areas clean */` markers and swept site-wide.) The old spammy footer "Popular Intersections" + "Service Areas" text-link column was removed site-wide (footer grid rebalanced to `repeat(4, 1fr) 1.4fr`).

**All 18 service pages: rebuilt from live WordPress copy (Aug 2026).** Every page in the Services menu now carries the copy from consultusdigital.com rather than from-scratch prototype copy, poured into one shared design system. Jake's rule: **take the live page's copy, don't merge it with the old staging copy.**

The treatment, applied identically to all 18:
- Live copy kept in substance and order. Promo furniture stripped: `$15,000 grant` banners, "Act Fast! Limited Spots Available for August", "SEE MORE" toggles, the guide-download form, the blog feed, the webinar bar, and duplicated sections (several live pages print 2-3 sections twice, a tabs/accordion bug).
- Errors fixed. The live pages carry real typos and bugs; each commit message lists what was corrected on that page. Recurring ones: bare "Consultus" in body copy, "#1" claims in titles, em dashes, scrambled heading levels (live uses H2 for card titles and H4 for section titles), and "SCHEDULE A RECOVERY CALL" for Discovery.
- Stale year-stamped stats dropped (e.g. "$4.62 billion by 2023", "reach $15 billion this 2022", "$0.91 CPC in 2023"). Evergreen figures kept.
- Standard blocks added to every page: the 4-card real-results band (Bow City +2900%, Harmony Resorts 6%, Ontario Business Central +41%, Spieth America $420K), the 5 client testimonials, a 4-item trust bar, and FAQPage + Service JSON-LD whose questions **must match visible headings verbatim**.

**The design system.** Each page has a prefixed copy of the Google Ads page's CSS block (`<style id="<pfx>-page-styles">`) so pages stay independent. Prefixes: gads, mads (meta), msa (microsoft), amz (amazon), seox (seo), lseo (local-seo), cmk (content), aeo, inf (influencer), pcr (creatives), wdv (web-dev), lpg (landing), crx (cro), abt (ab-testing), zcr (zoho-crm), mau (automation), ana (analytics), sen (sales-enablement).
Components: `-hero` (paper background, redesigned Sep 2026 from the CRO hero: keyword eyebrow, the live H2 with the accent word in `.sem` and a yellow `-hero-dot` period, lede, 4-item `-caps` icon row, two CTAs; right column is a tilted browser-window mockup `-mock` whose `-mock-body` shows THAT service's mechanism, e.g. the Zoho page shows a pipeline board, Analytics shows platform-reported vs actual revenue, with three pulsing `-note` chips. Built by `scratchpad/herobuild.py` (shell lifted from `cro/index.html`) plus `herorest.py` (the 15 per-service screens); regenerate if lost. The secondary CTA must point at `#<pfx>-results`. The older `svh-fan` photo-trio hero is in git history before commit 80efe6a), `-trust` bar, `-section` (+`.bone`/`.-dark`), `-section-head`, `-results-grid`, `-process` (4 or 5 cols), `-intro-grid` with `-photo`/`-photo-tag`, `-format-grid` (12-col; by default cards 4+5 span 6), `-benefit-grid` (2), `-metric-grid` (3; last child can span full width as a feature card), `-scope-grid` (3, or `.is-4` for 4), `-choice-grid`, `-complement-grid`, `-two-col` + `-text-panel`, `-why`/`-why-list`, `-testimonial-grid`, `-final`.
⚠️ **Grid arithmetic matters.** Card counts must divide evenly into the column count or a row is left orphaned. The builder takes per-page options for this (`format_wide`, `metric_feature`, `choice_cols`, `complement_cols`, `scope_is4`, `process_cols`). Any `.is-4` variant needs explicit overrides in BOTH responsive media queries or it won't collapse on mobile.

**Tooling** (`/tmp/.../scratchpad/svcbuild.py`, regenerate if lost): `port_css(prefix, **opts)` clones and re-prefixes the base CSS with the grid options; `build(page, prefix, main_file, title, desc, svc, type, qa, css_opts)` splices content between `<section class="svh-hero">` and `<footer>`, rewrites title/description, and replaces the Service + FAQ JSON-LD; `validate(page, prefix)` checks JSON-LD parses, FAQ questions appear in visible copy, zero em dashes, no bare "Consultus", one H1, no stray prefixes, and no broken internal links. Verify layout in the browser afterwards: grid row balance at 1440px, then no horizontal overflow at 768 and 375.

**Two pages had no live source** (`ab-testing/`, `analytics-attribution/`). Their existing staging copy was kept verbatim and only re-skinned. Their old "By the Numbers" bands were dropped: the figures were unsourced and the $420K claim collided with the real Spieth America $420K in the results band.

**Redirect hygiene.** Several `_redirects` entries pointed old live URLs at city pages rather than at the service page that now holds their content. Fixed: `/google-ads-management-toronto/`, `/facebook-advertising-toronto/`, `/instagram-marketing-toronto/` → their service pages. When moving live copy onto a staging page, always check where that live URL currently redirects.

**Amazon Ads page** (`amazon-ads/`): now carries the live `/amazon-ad-management-services/` copy (Sponsored Products / Brands / Display, Stores, Audio, Video, custom ads, DSP, Attribution). The earlier from-scratch version (ACOS/TACOS/MER scoreboard, Dragonscale angle) is in git history. Listed in the Services mega-menu (Acquire, after Microsoft Ads, box icon), the `svc-marquee` pill bar, and the homepage/`services/` `service-block` Acquire group.
...
**Dragonscale Supplies case study** (`case-studies/dragonscale-supplies/`): MER-first ecommerce story ("A niche brand, played at MER 13"). Counter 9.0 to 13.23 MER; substats +27% record month (revenue dollars intentionally omitted), 12.14x Google ROAS, 14.58x Meta ROAS, $0.49 CPC. Scoreboard: Briefcase Loadout 225 purchases 7.81x, Close Up + Features 156 at 10.51x, Windmill breakout, Advantage+ ~27x. dragonscalesupplies.com has X-Frame-Options DENY, NO live iframes. Numbers come from the monthly client deck and carry a [PROTOTYPE NOTE] to confirm client approval before going public. Assets in `assets/work/dragonscale/` (6 statics + loop-overheat/loop-dreamscape 8s loops). Includes **The Skin (CRO)** section: the storefront reskin we built (gamified dark Shopify rebuild, MAYHEM seasonal event engine, named colorway/hardware systems Wyvern/Basilisk/Leviathan/Orochi/Titan, welcome-offer popup, Dark Deals spend-threshold rewards, trust strip, 1,360 reviews, Hall of Flames UGC gallery, The Codex content hub). Rendered as a sticky dark storefront mock card built from the product statics (no iframe, XFO DENY); CRO uplift % left as a [PROTOTYPE NOTE] until the post-launch window closes. Real site: dragonscalesupplies.com.

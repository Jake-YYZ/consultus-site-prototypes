...
Components: `-hero` (dark, with the shared `svh-fan` photo trio), `-trust` bar, `-section` (+`.bone`/`.-dark`), `-section-head`, `-results-grid`, `-process` (4 or 5 cols), `-intro-grid` with `-photo`/`-photo-tag`, `-format-grid` (12-col; by default cards 4+5 span 6), `-benefit-grid` (2), `-metric-grid` (3; last child can span full width as a feature card), `-scope-grid` (3, or `.is-4` for 4), `-choice-grid`, `-complement-grid`, `-two-col` + `-text-panel`, `-why`/`-why-list`, `-testimonial-grid`, `-final`.
